...
void cross_dimensions_wow(map* m, int* i){
    if(strcmp(m[i[3]].mapa[i[2]][i[1]].contenido, "C")){
        if(i[1]== 5 && i[2]==2){
            i[1] = 0;
            i[2] = 2;
            for (int k = 0; k<9 ; k++){
                if (m[k].mapa == m[i[3]].der){
...
void generate_union(map* m, int* t){
    for (int i = 0; i<9 ; i++){
        if (i != t[3]){
            if (t[2] == 2 && t[1] == 5){
                if (!(strcmp(m[i].mapa[2][0].contenido,"B"))){
                    m[t[3]].der = m[i].mapa;
                    m[i].izq = m[t[3]].mapa;

                    strcpy(m[i].mapa[2][0].contenido, "C");
                    return;
                }
            }
            if (t[2] == 2 && t[1] == 0){
                if (!(strcmp(m[i].mapa[2][5].contenido,"B"))){
                    m[t[3]].izq = m[i].mapa;
                    m[i].der = m[t[3]].mapa;
                    strcpy(m[i].mapa[2][5].contenido, "C");
                    return;
                }
            }
            if (t[2] == 5 && t[1] == 2){
                if (!(strcmp(m[i].mapa[0][2].contenido,"B"))){
                    m[t[3]].sur = m[i].mapa;
                    m[i].norte = m[t[3]].mapa;
                    strcpy(m[i].mapa[0][2].contenido, "C");
                    return;
                }
            }
            if (t[2] == 0 && t[1] == 2){
                if (!(strcmp(m[i].mapa[5][2].contenido,"B"))){
                    m[t[3]].norte = m[i].mapa;
                    m[i].sur = m[t[3]].mapa;
                    strcpy(m[i].mapa[5][2].contenido, "C");
                    return;
                }
            }
